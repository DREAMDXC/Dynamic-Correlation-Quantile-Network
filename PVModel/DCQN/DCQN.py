...
#lib import
from Lib import evaluation_index
from Lib import dataload
from Lib import figureplot
...
# lower-triangular matrix with positive-valued diagonal entries
# cov matrix = L*L.T
...
def lossfunction_MLE_Copula(triangular_matrix,z):
    # triangular__matrix : batch size, dimension, dimension
    # lower-triangular factor of covariance, with positive-valued diagonal
    # z: real data: batch size, 1, dimension
    # https://pytorch.org/docs/stable/distributions.html#torch.distributions.multivariate_normal.MultivariateNormal.scale_tril
    # torch.distributions.multivariate_normal.MultivariateNormal  (It doesn't work very well )

    cov_matrix = torch.bmm(triangular_matrix,triangular_matrix.permute(0, 2, 1))
    # print(torch.det(cov_matrix))
...
def test(Test_input, Test_target):
    ...
    net1 = IQN_Decoder()
    net1.load_state_dict(torch.load('Forecast_Net{ep}.pkl'.format(ep=args.epoch), map_location='cpu'))

    net2 = DCN()
    net2.load_state_dict(torch.load('DCN_Net{ep}.pkl'.format(ep=args.epoch), map_location='cpu'))

    median_quantile = 0.5*torch.ones([Test_input.size(0), 1, args.farm_number, args.target_step])
    test_fore = net1(Test_input.unsqueeze(dim=2),median_quantile)
    test_fore = test_fore.cpu().detach().numpy()
    test_fore = np.clip(test_fore, a_min=0, a_max=1)

    matrix = net2(Test_input.unsqueeze(dim=2))
    cov = torch.bmm(matrix,matrix.permute(0,2,1)).detach().numpy()

    plt.figure(dpi=120)
    plt.title('cov matrix')
    sns.heatmap(data=cov[args.test_number, :, :], cmap=plt.get_cmap('Greens'))
    plt.show()

    sum_Quantile = torch.zeros([args.scenario_number, Test_input.size(0), 1, args.farm_number, args.target_step])

    for i in range(Test_input.size(0)):
        mean = np.zeros([args.target_step])
        Coupla_pdf = np.random.multivariate_normal(mean,cov[i,:,:],size=(args.scenario_number, 1, args.farm_number))
        Quantile = st.norm.cdf(Coupla_pdf)
        Quantile = torch.from_numpy(Quantile).float()
        sum_Quantile[:,i,:,:] = Quantile

    input_Quantile = sum_Quantile.reshape(args.scenario_number*Test_input.size(0), 1, args.farm_number, args.target_step)

    Test_input = Test_input.unsqueeze(dim=2).repeat(args.scenario_number,1,1,1)

    scenario = net1(Test_input,input_Quantile)

    scenario = scenario.cpu().detach().numpy() # size(batch,scenario,target_step)
    scenario = np.clip(scenario, a_min=0, a_max=1)
    scenario = scenario.reshape((args.scenario_number, Test_target.shape[0], 1, args.target_step), order='C').transpose(
        1, 0, 2, 3)

    return scenario, test_fore, target
...
if __name__ == '__main__':
    ...
    print('test sample num:',Test_input.shape[0])
    ...
    dei.main(forecast, target)

    pei = evaluation_index.probabilistic_evaluation_index()
    pei.main(scenario, target)

    sei = evaluation_index.scenario_evaluation_index()
